...
class MainModel extends CI_Model{
    ...
    public function get_medical_records($id){
        $query = "SELECT * FROM medical_records mr LEFT JOIN users u ON u.empid = mr.provider WHERE mr.patient_id = $id ORDER BY mr.time_created DESC";
        return $this->db->query($query)->result_array();
    }
    ...
    public function walkin_patient_name() {
        $query = "SELECT * FROM patient ORDER BY f_name";
        return $this->db->query($query)->result_array();
    }

    public function walkin_count() {
        $query = "SELECT * FROM walk_in_accepted WHERE id = 1";
        return $this->db->query($query)->result_array();
    }

    public function get_count_left() {
        $query1 = "SELECT COUNT(*) AS total_count FROM appointment WHERE DATE(created_at) = CURDATE()";
        $result1 = $this->db->query($query1)->result_array();
        foreach($result1 AS $res1){$count1 = $res1['total_count'];}

        $query2 = "SELECT walk_in_accepted FROM walk_in_accepted WHERE id  = 1";
        $result2 = $this->db->query($query2)->result_array();
        foreach($result2 AS $res2){$walk_in_accepted = $res2['walk_in_accepted'];}

        $total_count_all = (int)$walk_in_accepted - (int)$count1;
    ...
    public function get_privacy() {
        $query = "SELECT * FROM terms_conditions ORDER BY id ASC";
        return $this->db->query($query)->result_array();
    }
