...
class Main_Controller extends CI_Controller{
    ...
    public function add_child() {
        ...
        $data['vaccines']   = $this->main_mdl->get_vaccines();
    ...
    public function update_appointment() {
        ...
        $orig_service   = $this->input->post("orig_service");
        $comment        = $this->input->post("comment");
        $apt_select     = $this->input->post("apt_select");
    ...
    public function walk_in() {
        ...
        $data['walkin'] = $this->main_mdl->walkin_count();

        $data['count_left'] = $this->main_mdl->get_count_left();

        $this->load->view("walk_in", $data); 
    }
    
    public function update_walkin_count() {
        $walkin_count = $this->input->post("walkin_count");
        $update = "UPDATE walk_in_accepted SET walk_in_accepted = $walkin_count WHERE id = 1";
        $this->db->query($update);

        echo "Walk In Count Updated";
    }
    ...
    public function add_walkin() {
        ...
        $check_existing = "SELECT * FROM appointment WHERE date = '$date' AND time = '$time'";
        $res_exist = $this->db->query($check_existing);

        if($res_exist->num_rows() > 0){
            echo "There is an appointment Scheduled in this time. Please select other time";
        }else{
            $data = array(
                "name"       => $fname . " ". $lname,
                "service"    => $service,
    ...
    public function manage_privacy() {
        $data['get_privacy']  = $this->main_mdl->get_privacy();
    ...
    public function update_privacy() {
        ...
        $update_array = array("text_en" => $textareaValue);

        $this->db->where('id', $id);
        $this->db->update("terms_conditions", $update_array);

        echo "Updated terms_conditions";
    }
    ...
    public function add_vaccine() {
        ...
        $current_user   = $_SESSION['user_id'];
        $user_activity  = "Add Vaccine";
        $details        = "Added vaccine record id '$insert_id'";
    ...
    public function add_appointment_user() {
        ...
        if ($currentDate > $date || ($currentDate == $date && $currentTime >= $nightValid)){
            ...
            if($res_exist->num_rows() > 0){
                echo "There is other appointment in this time. Please select other time";
            }else{
