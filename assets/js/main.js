...
function update_walkin() {
    var walkin_count = $("#walkin_count").val()
    $.ajax({
        url: base_url + "Main_Controller/update_walkin_count",
        type: "POST",
        data: {walkin_count:walkin_count},
        success: function(res){
            final = res.trim()
            alert(final)
        }
...
function update_apt_status(id) {
    ...
    var comment         = $("#apt_comment_"+id).val()
...
function toggleCommentField(id) {
    ...
    if (selectedValue === "Canceled") {
        ...
        selectBox.setAttribute("id", "apt_select_" + id);
        selectBox.setAttribute("class", "form-control");

        // Create the initial blank option
        const initialOption = document.createElement("option");
        initialOption.text = "Select reason"; // Set the display text
        initialOption.value = ""; // Set the value to an empty string or any other default value
        selectBox.add(initialOption);

        // Add additional options as needed
        const option1 = document.createElement("option");
        option1.text = "Did not attend";
        option1.value = "Did not attend"; // Set the value for option 1
        selectBox.add(option1);

        const option2 = document.createElement("option");
        option2.text = "Change of Mind";
        option2.value = "Change of Mind"; // Set the value for option 2
        selectBox.add(option2);

        // Append the select box to the container
        commentBoxContainer.appendChild(selectBox);
    } else {
...
function toggleCommentField2(id) {
    ...
    if (selectedValue === "Canceled") {
        ...
        const commentBox = document.createElement("textarea");
        commentBox.setAttribute("placeholder", "Enter your comment here");
        commentBox.setAttribute("id", "apt_comment_"+id);
        commentBox.setAttribute("required", "true");
        commentBox.setAttribute("class", "form-control");
...
function update_appointment(idz) {
    ...
    var apt_select      = $("#apt_select_"+idz).val()
